...
クイズを選ぶと、回答を始める前に**回答方式**を選べます。どちらを選んでも学習体験の
好みに合わせられるよう用意した表示上の違いだけで、**採点・結果・履歴の扱いは完全に同じ**です。

- **一括表示**（従来の挙動）: すべての設問を 1 画面にまとめて表示し、最後に一度だけ回答します。
- **一問一答**: 設問を 1 問ずつ順番に表示し、「前へ」「次へ」で移動しながら回答します。

- **共通の採点・結果・履歴**: どちらのモードも同じ状態（設問ごとの選択）を共有し、送信時は
  **同一のサーバー側採点**（`POST /api/quizzes/{quizId}/submit`）を 1 回だけ呼び出します。表示される
  **結果画面も同一**で、採点後に **localStorage への履歴記録も同じ経路で 1 回だけ**行われます。
  つまり回答方式は表示の違いだけで、スコアや履歴の中身には一切影響しません。
- **追加依存ゼロ**: この機能はクライアント側の表示切り替えのみで実現しており、フロントエンドの
  依存関係は `react` と `react-dom` のみのままです（ルーターやその他ライブラリの追加はありません）。
